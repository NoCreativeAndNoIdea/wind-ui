...
import { useDisabled, useNamespace } from '@wind-ui/hooks'
    ...
    if (buttonColor) {
      ...
      if (props.plain) {
        styles = ns.cssVarBlock({
          'bg-color': isDark ? darken(color, 90) : color.tint(90).toString(),
        ...
        if (_disabled.value) {
          styles[ns.cssVarBlockName('disabled-bg-color')] = isDark
            ? darken(color, 90)
