...
export const generateTypesDefinitions = async () => {
  ...
  success('Added source files')

  typeCheck(project, [])
  success('Stricter type check passed!')
...
async function addSourceFiles(project: Project) {
  ...
  const filePaths = excludeFiles(
    await glob([globSourceFile, '!wind-ui/**/*'], {
      cwd: pkgRoot,
      absolute: true,
      onlyFiles: true,
    })
  ...
  await Promise.all([
    ...
    ...epPaths.map(async (file) => {
      const content = await fs.readFile(path.resolve(windRoot, file), 'utf-8')
      sourceFiles.push(
...
function typeCheck(project: Project, paths: string[]) {
  ...
  if (diagnostics.length > 0) {
    error(project.formatDiagnosticsWithColorAndContext(diagnostics))
